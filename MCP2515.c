...
//--------------------------- HAL Functions ---------------------------
static void MCP_startSpi(void)
{
    HAL_GPIO_WritePin(SPI_SA_CS_GPIO_Port, SPI_SA_CS_Pin, 0);
}

static void MCP_endSpi(void)
{
    HAL_GPIO_WritePin(SPI_SA_CS_GPIO_Port, SPI_SA_CS_Pin, 1);
}

static void MCP_transmitSpi(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
    HAL_SPI_Transmit(hspi, pData, Size, Timeout);
}

static void MCP_receiveSpi(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
    HAL_SPI_Receive(hspi, pData, Size, Timeout);
}

static uint32_t MCP_getTick(void)
{
    return (HAL_GetTick());
}

static void MCP_delay(uint32_t delay)
{
    HAL_Delay(delay);
}
// ---------------------------------------------------------------------
...
uint8_t MCP_getStatus(void)
{
    ...
    MCP_transmitSpi(&hspi3, &txData, 1, MCP_SPI_TIMEOUT);
    MCP_receiveSpi(&hspi3, &rxData, 1, MCP_SPI_TIMEOUT);
    MCP_endSpi();
...
static void MCP_setRegister(const REGISTER reg, const uint8_t value)
{
    ...
    txData[1] = reg;
    txData[2] = value;
    MCP_transmitSpi(&hspi3, txData, 3, MCP_SPI_TIMEOUT);
    MCP_endSpi();
}
...
static void MCP_setRegisters(const REGISTER reg, uint8_t values[], const uint8_t n)
{
    ...
    txData[0] = INSTRUCTION_WRITE;
    txData[1] = reg;
    MCP_transmitSpi(&hspi3, txData, 2, MCP_SPI_TIMEOUT);
    MCP_transmitSpi(&hspi3, values, n, MCP_SPI_TIMEOUT);
    MCP_endSpi();
}

static uint8_t MCP_readRegister(const REGISTER reg)
{
    uint8_t txData[2], rxData;
    MCP_startSpi();
...
static void MCP_readRegisters(const REGISTER reg, uint8_t values[], const uint8_t n)
{
    ...
    MCP_receiveSpi(&hspi3, values, n, MCP_SPI_TIMEOUT);
    MCP_endSpi();
}
...
static void MCP_modifyRegister(const REGISTER reg, const uint8_t mask, const uint8_t data)
{
    ...
    txData[2] = mask;
    txData[3] = data;
    MCP_transmitSpi(&hspi3, txData, 4, 10);
    MCP_endSpi();
}
...
static void MCP_prepareId(uint8_t *buffer, const bool ext, const uint32_t id)
{
...
MCP_ERROR MCP_setFilterMask(const MCP_MASK mask, const bool ext, const uint32_t ulData)
{
...
MCP_ERROR MCP_setFilter(const MCP_RXF num, const bool ext, const uint32_t ulData)
{
    MCP_ERROR res = MCP_setConfigMode();
    if (res != ERROR_OK)
    {
